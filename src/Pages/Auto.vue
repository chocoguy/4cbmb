<template>
    <Navbar />
    <div class="auto">
        <h2 class="orange-text" style="font-weight: bold; text-align: center; margin: 10px;">Auto</h2>
        <div class="OP-posts-container">
            <div v-if="OPPosts.length != 0" v-for="post in OPPosts" v-bind:key="post.Id" class="container-width">
                <OPPost v-bind:post="post" v-bind:board="'o'" />
            </div>
            <div v-else>
                ...
                <div class="page-selector" style="margin-left: 10px;" v-for="i in 10">
                    <a v-bind:href="'/o/' + i" class="orange-text" style="cursor: pointer; text-decoration: underline;">{{ i }}</a>
                </div>
                </div>
        </div>
...
export default defineComponent({
    ...
    methods: {
        async getNewPosts () {
            this.OPPosts = []
            var opPosts : FourCbmbPost[] = await FourCbmbService.GetOPPostsFromPage("o", this.page)
